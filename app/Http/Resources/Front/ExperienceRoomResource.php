...
class ExperienceRoomResource extends Resource
{
    ...
    public function toArray( $request )
    {
        ...
        return $this->price() + [
                ...
                'name'           => $this->name,
                'price'          => $this->price,
                'cover'          => $this->https($this->cover).'?imageMogr2/thumbnail/375x/format/jpg/size-limit/$(fsize)!/quality/70|imageslim',
                'type'           => $this->type,
...
//           if($item) return $item.'?imageView2/q/70/interlace/1|imageslim';
                if ($item) return $this->https($item).'?imageMogr2/thumbnail/375x/format/jpg/size-limit/$(fsize)!/quality/70|imageslim';
                return;
            }
        )
        ;
