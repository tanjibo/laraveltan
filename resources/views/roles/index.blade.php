    ...
    <div class="content">
        <div class="box">
            <div class="box-header">
                ...
                <a href="{{route('roles.create')}}" class="btn btn-success btn-sm pull-right">添加</a>
                <button class="btn btn-danger btn-sm pull-right"
                        onclick="delMass('{{route('roles.mass_destroy')}}')">删除选中
                </button>
            </div>
                ...
                <table class="table table-bordered table-striped">
                    ...
                    @foreach($model as $v)
                        <tr>
                            ...
                            <td>{{$v->guard_name}}</td>
                            <td>
                                @foreach ($v->permissions()->pluck('name') as $permission)
                                    <span class="label label-info label-many">{{ $permission }}</span>
                                @endforeach
                            </td>
                            <td>{{$v->created_at->diffForHumans()}}</td>
                            <td>
                                <a class="btn btn-sm btn-primary" href="{{route('roles.edit',$v)}}">编辑</a>
                                <a class="btn btn-sm btn-danger"
                                   onclick="del('{{route("roles.destroy",$v)}}')">删除</a>
                            </td>
                        </tr>
                    @endforeach
