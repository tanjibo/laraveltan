                    ...
                    >
                        <template slot-scope="props">
                            <el-popover trigger="hover" placement="top">
                                <p>预定时间: @{{ props.item.date }} @{{ props.item.time }}</p>
            ...
            <div class="box-body">
                ...
                <el-table-column label="ID" prop="id" sortable="custom">
                    <template slot-scope="scope">
                        <a :href="'/tearoom_booking/'+scope.row.id">@{{ scope.row.id }}</a>
                    </template>
                </el-table-column>
                <el-table-column label='空间' prop="experience_booking_rooms">
                    <template slot-scope="scope">
                       @{{scope.row.tearoom.name}}
                    </template>
                </el-table-column>
                <el-table-column label="日期"  sortable="custom">
                    <template slot-scope="scope">
                        <p>@{{scope.row.date}}</p>
                        <p> @{{ scope.row.time }}</p>
                    </template>
                ...
                <el-table-column label="人数" prop="peoples" sortable="custom">

                </el-table-column>
                <el-table-column label="价钱" prop="fee" sortable="custom">

                </el-table-column>
                <el-table-column label="实付" prop="real_fee" sortable="custom">
                    <template slot-scope="scope">
...
@section('javascript')
    <script>
        new Vue({
            ...
            mounted(){
                let url=laroute.route('tearoom_booking.index_api',{'page':this.currentPage})
            ...
            methods: {
                ...
                handleCurrentChange(val) {
                    ...
                    let url=laroute.route('tearoom_booking.index_api',{'page':val})
                    this.$http.post(url, this.querys).then(res => {
                        this.tableData = res.data;
                        this.perPage = res.per_page;
                        this.total = res.total;
                ...
                goBooking(){
                    window.location.href = "{{route('tearoom.index',['booking'=>true])}}"
                }
            }
        })
    </script>
